...
const range = (l: Lead) => `${gbp(l.low)}-${gbp(l.high)}`;
...
export function ownerSummary(l: Lead, s: Settings) {
  ...
  return `${flag}${l.score.toUpperCase()}: ${l.name} ${l.phone}. ${shortAddress(l)}. ${materialLabel(l, s)} ~${l.roofAreaM2}m2, ${range(l)}. ${TIMING[l.urgency]}.`;
}
...
function ownerEmailBody(l: Lead, s: Settings) {
  return [
    ...
    `Job: ${l.jobType}, ${materialLabel(l, s)} (${l.colour}), about ${l.roofAreaM2} m2 (${l.roofSource === "solar-api" ? "measured" : "estimated"})`,
    `Estimate shown: ${range(l)}. Earliest start shown: ${fmtMonth(l.earliestStart)}`,
    mapLink(l),
  ].join("\n");
}
...
export async function notify(l: Lead, s: Settings) {
  ...
  const emailText = fit
    ? `${text}\n\nProperty: ${shortAddress(l)}\nRoof: ${materialLabel(l, s)} (${l.colour}), about ${l.roofAreaM2} m2\n\nThis is an estimate only. We confirm the final price at a free inspection.\n\n${BUSINESS.name}\n${BUSINESS.phone}`
    : `${text}\n\n${BUSINESS.name}`;
